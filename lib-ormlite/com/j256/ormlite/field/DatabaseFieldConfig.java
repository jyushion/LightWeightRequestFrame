 ...
 * {@link DatabaseField} annotation or by direct Java or Spring wiring.
 * 
 * @author graywatson
 */
public class DatabaseFieldConfig {
	...
	/**
	 * For a foreign class which does not use the {@link DatabaseField}
	 * annotations, you need to inject the table configuration.
	 */
	 ...
	 *             {@link #setForeignCollectionOrderColumnName(String)}
	 */
	@Deprecated
	...
	/**
	 * @deprecated You should use
	 *             {@link #setForeignCollectionForeignFieldName(String)}
	 */
	@Deprecated
	...
	/**
	 * Create and return a config converted from a {@link Field} that may have
	 * one of the following annotations: {@link DatabaseField},
	 * {@link ForeignCollectionField}, or javax.persistence...
	 */
	public static DatabaseFieldConfig fromField(DatabaseType databaseType, String tableName, Field field) throws SQLException {
		...
		/*
		 * NOTE: to remove javax.persistence usage, comment the following lines
		 * out
		 */
		DatabaseFieldConfig config = JavaxPersistence.createFieldConfig(databaseType, field);
		if (config == null) {
			return getDefaultForField(databaseType, tableName, field);
		}
		// this can be null
		return config;
	}
	...
	public static DatabaseFieldConfig getDefaultForField(DatabaseType databaseType, String tableName, Field field) {
		DatabaseFieldConfig config = new DatabaseFieldConfig();
		config.fieldName = field.getName();
		if (databaseType.isEntityNamesMustBeUpCase()) {
			config.fieldName = config.fieldName.toUpperCase();
...
//		DatabaseField databaseField = field.getAnnotation(DatabaseField.class);
		...
		config.dataType = DataType.STRING;// databaseField.dataType();
		// NOTE: == did not work with the NO_DEFAULT string
		// String defaultValue = databaseField.defaultValue();
		// if (!defaultValue.equals(DatabaseField.DEFAULT_STRING)) {
		// config.defaultValue = defaultValue;
		// }
		config.width = 0;// databaseField.width();
		config.canBeNull = true;// databaseField.canBeNull();
		config.id = false; // databaseField.id();
		config.generatedId = false;// databaseField.generatedId();
		config.generatedIdSequence = null;// valueIfNotBlank(databaseField.generatedIdSequence());
		config.foreign = false;// databaseField.foreign();
		config.useGetSet = false;// databaseField.useGetSet();
		config.unknownEnumValue = null;// findMatchingEnumVal(field,
										// databaseField.unknownEnumName());
		config.throwIfNull = false;// databaseField.throwIfNull();
		config.format = null;// valueIfNotBlank(databaseField.format());
		config.unique = false;// databaseField.unique();
		config.uniqueCombo = false;// databaseField.uniqueCombo();

		// add in the index information
		config.index = false;// databaseField.index();
		config.indexName = null;// valueIfNotBlank(databaseField.indexName());
		config.uniqueIndex = false;// databaseField.uniqueIndex();
		config.uniqueIndexName = null;// valueIfNotBlank(databaseField.uniqueIndexName());
		config.foreignAutoRefresh = false;// databaseField.foreignAutoRefresh();
		config.maxForeignAutoRefreshLevel = DatabaseField.NO_MAX_FOREIGN_AUTO_REFRESH_LEVEL_SPECIFIED; // databaseField.maxForeignAutoRefreshLevel();
		config.persisterClass = VoidType.class;// databaseField.persisterClass();
		config.allowGeneratedIdInsert = false;// databaseField.allowGeneratedIdInsert();
		config.columnDefinition = null;// valueIfNotBlank(databaseField.columnDefinition());
		config.foreignAutoCreate = false;// databaseField.foreignAutoCreate();
		config.version = false;// databaseField.version();
		config.foreignColumnName = null;// valueIfNotBlank(databaseField.foreignColumnName());
		config.readOnly = false;// databaseField.readOnly();
