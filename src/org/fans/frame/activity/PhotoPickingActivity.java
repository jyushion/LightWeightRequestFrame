...
public class PhotoPickingActivity extends NetworkActivity {
		...
		if (resultCode == RESULT_OK) {
			...
			if (requestCode == TAKE_PHOTO) {
				...
				if (imageCramePath != null) {
					...
					// Filepath.toString());
					needFilePath = ImageUtils.compressNoLargePhoto(this, new File(imageCramePath), path, true);
				...
				try {
					...
					// fileDir.toString());
					needFilePath = ImageUtils.compressNoLargePhoto(this, realFile, fileDir, true);
	...
	public void startPhotoZoom(Uri uri, String tag) {
		...
		int widthSize = ViewUtils.getDisplayMetrics(this).widthPixels;
		int heightSize = ViewUtils.getDisplayMetrics(this).heightPixels;
		int defSize;
	...
	private File getImgFilePath() {
		...
		return Utils.getDiskCachePath(this, fileName);
	...
	public interface ImageFileNameGenerator {
		public String generateName(int opType);
	}
}
